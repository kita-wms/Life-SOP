...
// ─── TABS CONFIG ──────────────────────────────────────────────────────────────
// ─── CALENDAR TAB ─────────────────────────────────────────────────────────────
const CALENDAR_SRC = "https://calendar.google.com/calendar/embed?src=dwilliams3754%40gmail.com&ctz=America%2FChicago";

function CalendarTab() {
  const [view, setView] = useState("month");

  const views = [
    { id:"month",  label:"Month",  param:"month" },
    { id:"week",   label:"Week",   param:"week" },
    { id:"agenda", label:"Agenda", param:"agenda" },
  ];

  const src = `${CALENDAR_SRC}&mode=${view.toUpperCase()}&showTitle=0&showNav=1&showDate=1&showPrint=0&showTabs=0&showCalendars=0&showTz=0&bgcolor=%230d0d0d&color=%23c8f545`;

  return (
    <div>
      <Box style={{ borderColor:C.info+"44", marginBottom:16 }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:12 }}>
          <div>
            <Mono color={C.info}>GOOGLE CALENDAR</Mono>
            <div style={{ color:C.muted, fontSize:13, marginTop:4 }}>
              Add appointments directly in Google Calendar — they appear here automatically.
            </div>
          </div>
          <a
            ...
            style={{ background:C.info+"22", border:`1px solid ${C.info}44`, color:C.info, borderRadius:6, padding:"7px 14px", fontFamily:"'JetBrains Mono',monospace", fontSize:11, letterSpacing:1, textDecoration:"none", whiteSpace:"nowrap" }}
          >
            + ADD EVENT
          </a>
        </div>
      ...
      <div style={{ display:"flex", gap:6, marginBottom:16 }}>
        {views.map(v => (
          <button
            ...
            style={{ background: view===v.id ? C.accent+"22" : C.card, border:`1px solid ${view===v.id ? C.accent : C.border}`, color: view===v.id ? C.accent : C.muted, borderRadius:6, padding:"7px 16px", fontFamily:"'JetBrains Mono',monospace", fontSize:11, letterSpacing:1, cursor:"pointer", transition:"all .15s" }}
          >{v.label}</button>
        ))}
      </div>

      {/* Embedded calendar */}
      <div style={{ borderRadius:10, overflow:"hidden", border:`1px solid ${C.border}`, background:C.card }}>
        <iframe
          src={src}
          style={{ width:"100%", height: view==="agenda" ? 500 : 640, border:"none", display:"block", colorScheme:"dark" }}
      ...
      <Box style={{ marginTop:16 }}>
        <Mono color={C.muted}>QUICK TIPS</Mono>
        <div style={{ display:"flex", flexDirection:"column", gap:8, marginTop:10 }}>
          {[
            { tip:"Add recurring workouts", how:"In Google Calendar, create a repeating event Mon–Fri at 4:30am labeled with the day's workout." },
            { tip:"Add Sec+ study blocks", how:"Create a repeating event Mon–Fri at 5pm and 7:45pm labeled 'Sec+ Study'." },
            { tip:"Color code your life", how:"Use green for health/workouts, blue for work/study, purple for personal, red for non-negotiables." },
          ].map((t,i) => (
            <div key={i} style={{ padding:"10px 0", borderBottom:`1px solid ${C.border}` }}>
              <div style={{ fontWeight:600, color:C.text, fontSize:13, marginBottom:3 }}>{t.tip}</div>
              <div style={{ color:C.muted, fontSize:12, lineHeight:1.6 }}>{t.how}</div>
            </div>
          ))}
        </div>
      </Box>
